# Task 5 — Documentation Alignment + Validation

## Task Metadata
- Feature: strict-orchestrator
- Wave: 3
- Depends on: Tasks 3 and 4
- Objective: Align documentation and Sisyphus metadata with strict-orchestrator architecture, then run test/type gates.
...
### AGENTS.md
- Update agent reference table:
  - Sisyphus model -> `claude-opus-4-6`
  - Add prime-agent row
- Add command routing table section for autonomous command delegation targets
- Update model assignment section to reflect new routing expectations

### sisyphus.ts
- Update model references in comments/context so they no longer imply Sonnet orchestrator identity.

### sisyphus/SKILL.md
- Line containing model value should show Opus 4.6
- Permissions table must read: `orchestratorOnly`
- Delegation rules must include prime-agent usage for `/prime`

## Implementation Steps
1. Apply AGENTS.md updates in relevant sections only.
2. Update comment/model references in `sisyphus.ts`.
3. Update `sisyphus/SKILL.md` model and permissions rows, and delegation notes.
4. Run validation commands in `.opencode/`.
5. Capture pass/fail evidence in report.

## QA Scenarios
1. **AGENTS reference check**
   - Tool: Read
   - Expected: contains `claude-opus-4-6` for Sisyphus and includes `prime-agent`
2. **Skill table check**
   - Tool: Read
   - Expected: model row says Opus 4.6; permissions row says orchestratorOnly
3. **Runtime test check**
   - Tool: Bash
   - Expected: `bun vitest` returns 0 failures
4. **Typecheck check**
   - Tool: Bash
   - Expected: `npx tsc --noEmit` returns 0 errors

## Parallelization
- Parallelizable:
  - draft AGENTS.md updates and skill.md updates concurrently.
- Sequential required:
  - finalize docs before running tests/typecheck.

## Acceptance Criteria
- [ ] AGENTS.md updated with Opus model ref, prime-agent row, and command routing section
- [ ] sisyphus.ts references updated for Opus orchestrator context
- [ ] sisyphus/SKILL.md shows Opus 4.6 and `orchestratorOnly`
- [ ] `bun vitest` passes with 0 failures
...
## Rollback
- If test/type errors are introduced, revert only docs/metadata edits causing breakage and reapply minimally.
