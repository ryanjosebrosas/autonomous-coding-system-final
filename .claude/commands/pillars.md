...
Analyze the PRD and identify the fundamental infrastructure layers (pillars) that must be built in order. Each pillar is a coherent phase of work with clear gate criteria that must pass before the next pillar begins. Produces `.agents/specs/PILLARS.md`.
...
4. Scan the codebase for existing infrastructure:
   - Check `.agents/specs/BUILD_ORDER.md` if it exists (already-built specs indicate existing layers)
   - Check `.agents/specs/PILLARS.md` if it exists (preserve completed pillar status on re-run)
   - Scan top-level directories and key files to understand what already exists
...
This is non-negotiable. Every subsequent pillar depends on the foundation layer existing. If the PRD doesn't explicitly call out a data layer, create one from the data models in PRD Section 8. If no data models exist, Pillar 0 is still: scaffold + config + core types.
...
Key gate criteria:
  Pillar 0: {main gate test}
  Pillar 1: {main gate test}

Does this pillar structure look right? Any pillars to add, merge, split, or reorder?
```

Wait for explicit approval before writing `.agents/specs/PILLARS.md`. Adjust if requested.
...
Create `.agents/specs/` directory if needed. Write `.agents/specs/PILLARS.md` using this exact format:

```markdown
# Infrastructure Pillars
<!-- Generated: {date} | Source: PRD.md | Status: {N}/{total} complete -->

## Pillar 0: {Name} (e.g., "Data Infrastructure")
- **Status**: [ ] not started
- **Why first**: {what depends on this existing — be specific}
- **What this unlocks**: {what becomes possible once this pillar is done — specific capabilities}
- **What breaks without it**: {what fails or can't be built if this pillar is missing}
...
- **Gate criteria**:
  ...
  - [ ] Validation pyramid clean (lint + types + tests)

## Pillar 1: {Name}
- **Status**: [ ] not started
- **Why next**: {what this enables — what becomes possible after this pillar}
- **What this unlocks**: {what becomes possible once this pillar is done}
- **What breaks without it**: {what fails if this pillar is missing or incomplete}
...
- **Gate criteria**:
  ...
  - [ ] Validation pyramid clean

## Pillar 2: {Name}
- **Status**: [ ] not started
- **Why next**: {what this enables}
- **What this unlocks**: {what becomes possible once this pillar is done}
- **What breaks without it**: {what fails if this pillar is missing}
- **Scope**: {what's included}
- **Not included**: {boundary}
- **Depends on**: Pillar 0, Pillar 1
- **Estimated specs**: ~{N} (light: {n}, standard: {n}, heavy: {n})
- **Gate criteria**:
  - [ ] All specs for this pillar marked [x] in BUILD_ORDER.md
...
| # | Pillar | Depends On | Est. Specs | Enables | Status |
|---|--------|-----------|------------|---------|--------|
| 0 | {Data Infrastructure} | None | ~N | {what Pillar 1 can do} | [ ] |
| 1 | {name} | 0 | ~N | {what Pillar 2 can do} | [ ] |
| 2 | {name} | 0, 1 | ~N | {what Pillar 3 can do} | [ ] |
```
...
After writing `.agents/specs/PILLARS.md`:

1. Confirm the file path
2. Print the Pillar Order Summary table
3. State the next step:
   ```
   Next: /decompose — break each pillar into individual specs with dependency order.
   Run /decompose to continue.
   ```
...
1. Read existing `.agents/specs/PILLARS.md`
...
- **Pillar 0 is always data/infrastructure.** This is the one non-negotiable rule. Even "simple" projects need a type-safe, tested foundation before building services on top.
- **"What breaks without it" is the litmus test.** If removing a pillar doesn't break anything, it's not a real pillar — merge it into an adjacent one.
