...
Read these files:
- `.agents/specs/BUILD_ORDER.md` — spec list with completion status
- `.agents/specs/build-state.json` — cross-session context (if exists)
...
2. Run a quick syntax check on those files using the project-configured lint command
...
Run the project-configured validation pyramid:
...
## Step 4b: Pillar Health (when .agents/specs/PILLARS.md exists)
...
If `.agents/specs/build-state.json` exists, cross-check it against reality:

1. Does `completed` list match `[x]` specs in `.agents/specs/BUILD_ORDER.md`?
2. Are `patternsEstablished` still valid? (quick grep for pattern usage)
3. Is `currentPillar` accurate?
...
Estimated: {remaining} specs left
...
## Step 5b: Archon Sync (if connected)

If Archon MCP is available:
1. Check project exists in Archon
2. Compare local task status with Archon task status
3. Report any drift:
   - Tasks marked done locally but not in Archon
   - Tasks marked done in Archon but not locally
4. Offer to sync: "Sync local state to Archon? [y/n]"

If Archon not connected: Skip this step.
