...
5. Saves the report to `.agents/reviews/`
...
```bash
# Fix all issues
/code-review-fix .agents/reviews/feature-review.md
```

```bash
# Fix only critical and major issues
/code-review-fix .agents/reviews/feature-review.md critical+major
```

```bash
# Fix issues in specific files only
/code-review-fix .agents/reviews/feature-review.md src/auth/
```
...
**Output**: Saved to `.agents/reports/` for later use by `/system-review`.
