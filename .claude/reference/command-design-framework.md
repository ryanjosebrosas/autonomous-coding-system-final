...
Agent researches codebase → creates `.agents/plans/{feature}-plan.md` → output optimized for `/execute` to consume. Effective because: explicit file paths (`src/auth/jwt.py:45-62` not "the auth module"), exact commands (copy-paste ready), validation checkpoints per task, complete context (zero assumptions).
...
6. Create plan document and save to `.agents/plans/{feature}-plan.md`
