**HARD RULE — Opus Never Implements** — Claude Opus (this model) handles ONLY planning, architecture, orchestration, exploration, and strategy. ALL implementation (file edits, code writing, refactoring) MUST be dispatched to T1-T5 models via the dispatch tool through opencode serve. Opus writing code directly is a violation. No exceptions. **Graceful degradation**: If dispatch tools are unavailable, the primary session model (whichever model is running this session) handles implementation work directly — this is NOT a violation. A violation is only when dispatch IS available and implementation is done locally instead of dispatching. When dispatch is unavailable, proceed with the task using the primary model's own tools.
...
**Valid implementation path**: Plan in .agents/plans/ -> dispatch(mode:"agent", provider:"bailian-coding-plan-test", ...) -> T1 edits via native agent mode -> T2 review via dispatch(taskType:"code-review")
