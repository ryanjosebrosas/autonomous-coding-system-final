...
Look for code review artifacts in `.agents/reviews/`.

**If NOT run:**
```
Code review not detected. Running code review now...
```
Run `/code-review` on the current changes. Use findings in Step 4 (Code Quality Score).
...
**Read plan file** and assess each section:
...
**Generate lessons from the execution:**
...
Calculate using weighted components:
...
- **Plan Adherence (40%)** = (File Adherence % + Pattern Compliance %) / 2
  - File Adherence % = (planned files modified / total planned files) × 100
  - Pattern Compliance % = (referenced patterns found / total referenced) × 100

- **Plan Quality (20%)** = Plan Quality Score from Step 2 (already /10)

- **Divergence Justification (30%)** = (Good divergences / Total divergences) × 10
  - If 0 divergences: score = 10
  - If all divergences justified: score = 10
  - If all divergences problematic: score = 0

- **Code Quality (10%)** = 10 - (Critical×2 + Major×1 + Minor×0.5)
  - Min: 0, Max: 10
  - From Step 4 code review findings
...
Divergence {N}:
  ...
  System Fix: {which asset to update: command/template/section/memory}
```

**Divergence Justification Score:** __/10
- (Good divergences / Total divergences) × 10
- If 0 divergences: 10/10

**Root Cause Summary:**
- Unclear plan: {N} occurrences
- Missing context: {N} occurrences
- Missing validation: {N} occurrences
- Manual step repeated: {N} occurrences
- Other: {N} occurrences
...
- Add gotcha: "{pitfall to remember}"
- Add pattern: "{successful approach to replicate}"
- Add decision: "{architecture/tech choice with rationale}"
